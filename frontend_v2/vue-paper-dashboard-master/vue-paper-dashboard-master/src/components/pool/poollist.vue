  ...
  <div>
    ...
    -->
      ...
      <li class="cards__item" v-for="pool in extendpools" :key="pool.id">
        <div class="card">
          <div v-if="pool.responsibility === '디자인'" class="card__image card__image--designer"></div>
          <div v-if="pool.responsibility === '개발'" class="card__image card__image--developer"></div>
          <div v-if="pool.responsibility === '기획'" class="card__image card__image--head"></div>
          <div class="card__content">
            ...
            <div class="inter">
            <div v-for="(interest, idx) in pool.interest" :key="idx" style="margin:2px; display:inline-block">
                    #{{interest.interest}},
...
<hr>
              ...
              <i class="fa fa-map-marker emo" > </i> {{pool.address2}}
              </div>
  ...
  <button v-b-modal="'test-'+pool.id" style="margin:0px; padding:0px; border:0; outline:0; font-size:22px; background-color:white;"><i class="fa fa-tasks emo" v-b-tooltip.hover title="프로젝트 목록"></i></button>
  ...
  <b-modal :id="'test-'+pool.id" title="참여한 프로젝트 목록" hide-footer>
    ...
    <div v-if="pool.projects == null">
      아직 참여한 프로젝트가 없습니다. 이번 기회에 함께 해보는건 어떨까요?
      </div>
  </b-modal>
</div>

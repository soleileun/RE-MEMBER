      ...
      <div class="modal-content">
        ...
        <table>
          <tr v-for="project in projects" :key="project.pid">
            <td>
              <input type="radio" name="pjtList" :value="project.pid" />
              {{project.pjtName}}
            </td>
          </tr>
        </table>제목 :
        ...
        <br />내용
        <br />
        <textarea name id cols="30" rows="10" v-model="wrecruit.contents" placeholder="내용을 입력하세요"></textarea>
        <br />
    ...
    <table id="table1">
      ...
      <tbody>
        <tr v-for="recruit in recruits" :key="recruit.rnum">
          ...
          <td>{{recruit.pid}}</td>
          <td>
            <router-link :to="'/recruit/recruitdetail/' + recruit.rnum">{{recruit.title}}</router-link>
          </td>
          <td>{{recruit.makeId}}</td>
          <td>{{recruit.makeDay}}</td>
        </tr>
      </tbody>
    </table>검색 다양한 기준 생각해볼 것 + 여기도 게시판처럼 CRUD 해야함
    <br />

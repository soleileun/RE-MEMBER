	...
	<div class="card">
		...
		<div class="card-body">
			...
			<div class="card-body-header">
				<h1>{{project.pjtName}}</h1><p class="card-body-hashtag">#이슈태그</p>
...
export default {
  ...
  computed :{
    pjtcnt(){
      console.log(this.project.pid + '번 프로젝트 : '+ this.$store.state.projectstore.pjtcnt);
      return this.$store.state.projectstore.pjtcnt;
    }
  },
  created(){
    console.log(this.project.pid);
    this.$store.dispatch(Constant.GET_CURRENT_MEMBER_COUNT, {pid : this.project.pid})
  },
  props:{
    project : {
      type : Object,
  ...
  methods:{
    popup(){
            //배포 주소로 바꾸기
            var url = "http://localhost:9000/project/projectdetail/" + this.project.pid;
...
.card-body-footer {  	position : absolute;   	margin-top: 15px;  	margin-bottom: 6px;    bottom: 0; 
    width: 314px;
    font-size: 14px;
    color: #9FA5A8;
    padding: 0 15px;

<template>
  ...
  <table>
    ...
    <tbody>
      <tr v-for="free in frees" :key="free.bno">
        <td>{{free.bno}}</td>
        <!-- bno 쿼리스트링 달아서 분기 -->
        <td>  <router-link :to="'/notice/detailnotice/' + free.bno">{{free.btitle}}</router-link></td>
        <td>{{free.bwriter}}</td>
        <td>{{free.makeDay}}</td>
      </tr>
    </tbody>
  ...
  <!-- 디비에 들어갈 조건들 : {{selected}} + {{searchWord}} <br> -->
    ...
    <input type="text" placeholder="검색어를 입력하세요" id="searchWord2"/>
    <button v-on:click="searchNotice">검색</button>
